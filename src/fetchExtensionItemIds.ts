...
const fetchExtensionItemIds = async (
  ...
  const classQuery = `
    ...
    WHERE d.deptype = 'e'
  `;
  const classResult = await pg.query<{ oid: number }>(classQuery);
  const extClassOids = classResult.rows.map(({ oid }) => oid);
  ...
  const typeQuery = `
    ...
    WHERE d.deptype = 'e'
  `;
  const typeResult = await pg.query<{ oid: number }>(typeQuery);
  const extTypeOids = typeResult.rows.map(({ oid }) => oid);
  ...
  const procQuery = `
    ...
    WHERE d.deptype = 'e'
  `;
  const procResult = await pg.query<{ oid: number }>(procQuery);
  const extProcOids = procResult.rows.map(({ oid }) => oid);
